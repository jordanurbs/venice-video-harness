...
29. **Always run cut-qa after every assembly / edit render.** The `cut-qa` agent runs programmatic checks (aspect, visual jump, VO truncation, **dialogue/VO overlap** — assert no two spoken clips play simultaneously, see rule 33, lighting, audio pop, subtitle overlap) at cut boundaries. Max 3 fix iterations before surfacing to the user. Applies to BOTH the generation-pipeline assembler and the editing-pipeline render.
...
32. **For character-dialogue shots, keyframe Wan 2.7 i2v from a Seedance R2V output — not from a panel.** Wan 2.7 i2v has no `reference_image_urls` capability; its only identity anchor is the single `image_url` keyframe. A panel-derived keyframe drifts mid-clip because the panel was generated without strong character anchoring. The validated pipeline: (a) render the shot via `seedance-2-0-reference-to-video` with all 4 character refs and a tight 5-part prompt (skip lip-sync at this stage); (b) `ffmpeg -i shot-NNN-seedance-r2v.mp4 -frames:v 1 shot-NNN-keyframe.png`; (c) render via `wan-2-7-image-to-video` with that keyframe as `image_url` + the dialogue MP3 (≥3s, pad with trailing silence if needed) as `audio_url`. Result: locked identity from frame 0 + lip-sync. Cost ~$0.85/shot total. Apply when: character is on-screen, dialogue exists, motion is low/medium. Skip when: no dialogue (just use Seedance R2V), high motion (Wan drifts regardless of keyframe — use Seedance R2V only and accept no lip-sync), or multi-character dialogue with different speakers (use Wan 2.7 R2V `per_reference_audio` instead).
33. **Schedule dialogue/VO with a global no-overlap scheduler driven by MEASURED clip durations — never by the script's planned shot lengths.** At assembly time, two audio clips may never play at once unless they are intentionally layered (e.g. a music bed vs a line). Build the schedule against the actual `ffprobe` duration of each rendered/normalized segment, not the `duration` field in the script (the rendered clip is almost always shorter or longer than its planned slot). Algorithm: walk shots in timeline order keeping a single `nextFreeSec` cursor; place each line at `max(shotStart + lead, nextFreeSec + gap)`; set `nextFreeSec = placedStart + audioDur`; use one cursor for ALL spoken lines regardless of speaker (narrator AND character) — a per-speaker cursor is the classic bug (see anti-pattern 19). Keep a small `gap` (≈0.2–0.3s) between consecutive lines. SFX and the music bed are exempt (they are meant to underlay). After mixing, verify with the cut-qa overlap check.
34. **Author VO so each line fits its shot, and when it can't, extend the picture — never let audio bleed into the next shot.** A line of TTS runs ≈2.3–2.7 words/sec plus ~0.4s lead-in; budget `shotSeconds × 2.4` words and write to it. If a finished line is longer than its shot's video, the assembler must (a) extend that shot by freezing/holding its last frame to cover the audio (`ffmpeg tpad=stop_mode=clone:stop_duration=…`), or (b) the line must be shortened/split — it must NOT spill onto the next shot. Long narrator lines over short establishing shots are the usual offender; either tighten the narration or hold the frame. This is the authoring complement to the scheduler in rule 33.
35. **Re-anchor every separately-rendered shot to the SAME locked references and restate the character's invariant traits in every prompt.** Identity, scale, palette, and wardrobe drift across independently generated shots even when the story is continuous. For each shot pass the identical canonical `reference_image_urls` (not a frame grabbed from a different shot), and repeat the character's fixed traits inline every time (markings, hair color, costume, and **relative size** — e.g. "as tall as the boy"). Size is a trait the model forgets most: if a character's scale changed in-story (grew/shrank), encode a `sizeState` per shot and state it explicitly. Prefer Seedance native multi-shot (rule 21) for consecutive beats precisely because identity/scale/lighting hold within one generation; across separate renders, the per-prompt trait restatement is what holds them together. Verify drift on a contact sheet of first-frames before assembling (see anti-pattern 20).
36. **Validate the reference-image COUNT against the target model's per-model cap before queueing — clamp, don't overflow.** Every R2V model has a hard maximum on `reference_image_urls`, and it differs by model: Seedance 2.0 R2V = 4, `happyhorse-1-0-reference-to-video` = 5, Kling O3 R2V uses structured `elements` instead. A render that appends a book/scene panel to a full character-ref set silently pushes the array over the cap and the queue call 400s (`"Reference images must be at most N for this model (received N+1)"`) — the whole batch fails, not just the extra ref. When a shot's total refs exceed the cap: anchor on the most identity-critical image as `image_url` (the book/scene panel), then fill `reference_image_urls` with the LEAD characters up to `cap - 1`, dropping the least-important refs (background/minor characters like parents) rather than truncating blindly. Read the cap from `getVideoModel()` in `src/venice/models.ts` and `.slice(0, cap)` before building the request body — never assume all models share Seedance's limit of 4. See anti-pattern 21.
...
### 21. Reference-Image Overflow And Generic-Costume Drift On Distant Characters
Two render-driver bugs hit while re-rendering a book-to-video episode; both are model-behavior lessons, not project quirks.

**21a — Ref-count overflow 400s the whole render.**
**Symptom:** Two bundles targeting `happyhorse-1-0-reference-to-video` failed at queue time with `"Reference images must be at most 5 for this model (received 6)"`. The bundle passed a book-page panel PLUS all 5 character references (6 total). The batch aborted; the other shots in the batch were fine.
**Root cause:** The render driver appended the anchoring panel to a fixed full character-ref set without checking the model's per-model `reference_image_urls` cap. The cap is model-specific — Seedance R2V allows 4, happyhorse allows 5 — so a set sized for one model overflows another.
**Fix:** In the model branch, read the cap and clamp: set `image_url` to the book/scene panel, then `reference_image_urls = leadCharRefs.slice(0, cap - 1)`, dropping non-essential refs (parents/background characters) first. Never build the ref array assuming Seedance's limit of 4 applies everywhere. See rule 36.

**21b — Distant/costumed characters drift to generic props when the prompt is vague.**
**Symptom:** A T2V shot of small distant costumed characters rendered them in wrong, random costumes — pirate hats, wizard hats, glowsticks — none of which are in the story.
**Root cause:** The prompt described them generically ("person-sized costumed geckos") and they were small background silhouettes, so the model had no anchor and invented Halloween-costume clichés. Distant figures get no R2V identity pass, so the *text* is their only constraint.
**Fix:** Name each costume exactly in the prompt (e.g. "one in an orange PUMPKIN costume, one in a white GHOST sheet with a flame tail, one with black BAT wings and a red heart") and add the invented props to the negatives ("NO pirate hats, NO wizard hats, NO glowsticks"). Keep them "small, distant, dim, not the focus." This is the silhouette-character complement to rule 16 and the per-prompt trait restatement of rule 35.
**Files:** project render scripts; prompt builders that emit `silhouetteCharacters`.
