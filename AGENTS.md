...
**Fix:** (a) `video-generator.ts` now sets `body.aspect_ratio` for every `text-to-video` model as well. (b) `extractLastFrame` probes the `v:0` stream duration and steps back in widening offsets (0.1/0.3/0.6/1.0s) until the PNG actually exists on disk, throwing otherwise. (c) No code fix — run watch mode with `--no-chain` when the loop's shots feature human faces (all shots render t2v; multi-cam cutting grammar survives), or use create mode whose R2V references are generated sheets (face-bearing R2V refs on H3 Max not yet verified either way). The re-queue-on-poll-failure cycle needs a guard (e.g. mark the take permanently failed after N consecutive retrieve 500s on the SAME queue_id instead of clearing the record); until then, watch the log for repeated `An unknown error occurred` on one shot and stop the loop manually.
**Files:** `src/mini-drama/video-generator.ts`, `src/mini-drama/loop-engine.ts`
