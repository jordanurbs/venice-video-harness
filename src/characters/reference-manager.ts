 ...
 *
 * `seedream-v5-lite` is required because Seedance 2.0 (the harness's default
 * video family) blocks any request whose reference images were generated by
 * a different family. Override by passing `model` into `generateReferences`
 * or by setting `series.videoDefaults.imageDefaults.generationModel`.
 */
...
export class ReferenceManager {
  ...
  async saveReferences(lock: CharacterLock): Promise<void> {
    ...
    for (const angle of REFERENCE_ANGLES) {
      ...
      if (modelUsed) {
        await writeImageProvenance(imagePath, modelUsed);
      }
    }
